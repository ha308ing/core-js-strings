...
function removeFirstOccurrences(str, value) {
  ...
  const str2 = str.substring(index + value.length, str.length);
  return str1 + str2;
}
...
function isPalindrome(str) {
  ...
  const f = str.replaceAll(/\W/g, '');
  let i = 0;
  while (i < n) {
    const a = f[i].toLowerCase();
    const b = f[f.length - 1 - i].toLowerCase();
    if (a !== b) return false;
    i += 1;
  }
  return true;
...
function encodeToRot13(str) {
  ...
  while (i < a.length) {
    const l = a[i].toLowerCase();
    const index = o.indexOf(l);
    if (index >= 0) {
      const isLowerCase = l === a[i];
      let newIndex = index + 13;
      if (newIndex >= o.length) newIndex -= o.length;
      const newLetter = isLowerCase ? o[newIndex] : o[newIndex].toUpperCase();
      r += newLetter;
    } else {
      r += a[i];
    }
